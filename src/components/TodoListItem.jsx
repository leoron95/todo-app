...
export const TodoListItem = ({
    ...
    return (
        <div key={id} className={`flex flex-row  justify-start border-b-[1px] ${toggleDark ? 'bg-[#25273c] border-b-[#484b6a] ' : ''}   h-14 md:h-16 w-[330px] first:rounded-tr-[5px] first:rounded-tl-[5px] sm:w-[500px] md:w-[600px]  items-center ${snapshot.isDragging && toggleDark ? 'bg-gray-600' : ''} ${snapshot.isDragging && toggleDark == false ? 'bg-gray-400' : ''} `}>
            ...
            <div>
                <div onClick={()=> checkTodo(id, completed)} aria-label='Check button' className={` ml-5 flex cursor-pointer  items-center justify-center sm:p-[1px]  rounded-full sm:hover:bg-gradient-to-r from-[#57ddff] to-[#c058f3] rotate-45  border-[1px] ${toggleDark ? 'bg-[#25273c] border-[#484b6a] ' : 'bg-white border-[#9394a5] '} ${completed ? 'border-none' : ''}  `}>
                ...
                <div className='flex w-3 ml-auto mr-5 sm:w-4 '>
                    <button onClick={()=>deleteTodo(id)} className='flex' aria-label='Close button'>
